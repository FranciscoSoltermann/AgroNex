...
export default function DashboardHome() {
    ...
    if (loading) {
        return (
            <div className="flex-1 min-h-0 flex flex-col gap-2.5 animate-pulse h-full overflow-hidden pb-1">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 shrink-0">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 h-20 border border-gray-100 dark:border-gray-800" />
                    ))}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 flex-1 min-h-[170px]">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 border border-gray-100 dark:border-gray-800" />
                    ))}
                </div>
                <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 flex-1 min-h-[170px] border border-gray-100 dark:border-gray-800" />
                <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 h-28 shrink-0 border border-gray-100 dark:border-gray-800" />
            </div>
        );
    }

    return (
        <div className="flex-1 min-h-0 flex flex-col gap-2.5 animate-in fade-in duration-500 h-full overflow-y-auto pr-1 pb-1 custom-scrollbar">
            {/* Stats — 3 cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 shrink-0">
                <StatCard
                    label="Total Ha"
                ...
                <StatCard
                    ...
                    icon={<Grid2x2 size={13} className="text-indigo-600" />}
                    iconBg="bg-indigo-50"
                />
                <StatCard
                    ...
                    icon={<RefreshCw size={13} className="text-teal-600" />}
                    iconBg="bg-teal-50"
                />
            </div>

            {/* Alertas de Inventario + Gastos por Categoría + Actividades Recientes */}
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 flex-1 min-h-[170px] overflow-hidden">
                {/* Alertas de Inventario */}
                {(!userRole || userRole !== "EMPLEADO" || userPermisos.includes("GESTION_INVENTARIO")) && (
                    <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col h-full min-h-0 overflow-hidden">
                        <div className="flex items-center justify-between mb-2 shrink-0">
                            <div className="flex items-center gap-2">
                                <div className="w-7 h-7 bg-orange-50 rounded-lg flex items-center justify-center">
                                    <AlertTriangle size={15} className="text-orange-500" />
                                </div>
                                <h3 className="text-[13px] font-bold text-gray-900 dark:text-gray-100">Alertas de Inventario</h3>
                            </div>
                            <a href="/dashboard/inventario" className="text-[10px] font-bold text-[#2D6A4F] hover:underline flex items-center gap-1">
                        ...
                        {lowStockItems.length === 0 ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 text-xs font-medium">
                                <Package size={22} className="mb-1 text-emerald-500" />
                            ...
                            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                                {lowStockItems.map((item) => {
                                    ...
                                    return (
                                        <div key={item.idInsumo} className="flex items-center gap-2.5">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between mb-0.5">
                                                    <p className="text-[11px] font-bold text-gray-900 dark:text-gray-100 truncate">{item.nombre}</p>
                                                    <span className={`text-[11px] font-black ${textColor} flex-shrink-0 ml-2`}>
                    ...
                    <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col h-full min-h-0 overflow-hidden">
                        <div className="flex items-center gap-2 mb-1 shrink-0">
                            <div className="w-7 h-7 bg-violet-50 rounded-lg flex items-center justify-center">
                                <PieChartIcon size={15} className="text-violet-500" />
                            </div>
                            <h3 className="text-[13px] font-bold text-gray-900 dark:text-gray-100">Gastos por Categoría</h3>
                        </div>
                        {gastosPorCategoria.length === 0 ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 text-xs font-medium">
                                <DollarSign size={22} className="mb-1 text-gray-300" />
                                No hay gastos registrados.
                            </div>
                        ) : (
                            <div className="flex-1 min-h-0 flex flex-col items-center justify-between gap-1">
                                ...
                                <div className="w-full space-y-1 max-h-20 overflow-y-auto custom-scrollbar pr-1 shrink-0">
                ...
                <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-3 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col h-full min-h-0 overflow-hidden">
                    <h3 className="text-[13px] font-bold text-gray-900 dark:text-gray-100 mb-1.5 shrink-0">Actividades Recientes</h3>
                    <div className="flex-1 min-h-0 flex flex-col justify-around space-y-1 overflow-hidden">
                        {actos.length === 0 ? (
                            <p className="flex-1 flex items-center justify-center text-gray-400 text-xs">No hay actividades recientes.</p>
                        ) : actos.slice(0, 4).map((act, i) => {
                            const config = getActividadConfig(act.tipoActv);
                            return (
                                <div key={act.idActividad || i} className="flex items-start gap-2.5">
                                    <div className={`w-7 h-7 rounded-lg ${config.bg} flex items-center justify-center flex-shrink-0`}>{config.icon}</div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[11px] font-bold text-gray-900 dark:text-gray-100 truncate">{act.tipoActv}</p>
                                    ...
                                    <span className="text-[9.5px] text-gray-400 font-medium flex-shrink-0">{act.fecha}</span>
                                </div>
                            );
                        })}
                    </div>
                    <a href="/dashboard/lotes" className="mt-1.5 w-full flex items-center justify-center gap-1 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-[10px] font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-200 transition-all text-center shrink-0">
            ...
            {(!userRole || userRole !== "EMPLEADO" || userPermisos.includes("GESTION_FINANZAS")) && (
                <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-2.5 sm:p-3 shadow-sm border border-gray-100 dark:border-gray-800 flex-1 min-h-[170px] flex flex-col overflow-hidden">
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between mb-1 shrink-0">
                        <div className="min-w-0">
                            <h3 className="text-[13px] font-bold text-gray-900 dark:text-gray-100">Rentabilidad y Flujo Financiero</h3>
                        ...
                        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-md p-0.5 gap-0.5 shrink-0 self-start">
                            {["Por Campaña", "Flujo Temporal"].map(mode => (
                                <button
                                    key={mode}
                                    type="button"
                                    onClick={() => setChartMode(mode)}
                                    className={`px-2 py-0.5 rounded text-[8.5px] font-bold uppercase tracking-wide transition-all ${chartMode === mode ? "bg-[#2D6A4F] text-white shadow-sm" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"}`}
                                >
                                    {mode}
                                </button>
                            ))}
                    ...
                    <div className="grid grid-cols-3 gap-2 my-1 p-1.5 rounded-lg bg-gray-50 dark:bg-[#15191e] border border-gray-100 dark:border-gray-800 shrink-0">
                        <div className="flex flex-col min-w-0">
                            <span className="text-[8.5px] font-bold uppercase tracking-wider text-gray-400">Total Invertido</span>
                            <span className="text-[11px] sm:text-[12px] font-black text-[#E07A5F] truncate">
                                {symbol} {summaryFinanzas.totalCostos.toLocaleString("es-AR")}
                            </span>
                        </div>
                        <div className="flex flex-col min-w-0">
                            <span className="text-[8.5px] font-bold uppercase tracking-wider text-gray-400">Total Liquidado</span>
                            <span className="text-[11px] sm:text-[12px] font-black text-[#2D6A4F] dark:text-[#52B788] truncate">
                                {symbol} {summaryFinanzas.totalIngresos.toLocaleString("es-AR")}
                            </span>
                        </div>
                        <div className="flex flex-col min-w-0">
                            <span className="text-[8.5px] font-bold uppercase tracking-wider text-gray-400">Margen Neto Total</span>
                            <div className="flex items-center gap-1 min-w-0">
                                <span className={`text-[11px] sm:text-[12px] font-black truncate ${summaryFinanzas.margenNeto >= 0 ? "text-[#2D6A4F] dark:text-[#52B788]" : "text-red-500"}`}>
                                    {summaryFinanzas.margenNeto >= 0 ? "+" : ""}{symbol} {summaryFinanzas.margenNeto.toLocaleString("es-AR")}
                                </span>
                                {summaryFinanzas.roi > 0 && (
                                    <span className="hidden sm:inline-block text-[7.5px] px-1 py-0.5 rounded bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 font-bold shrink-0">
                    ...
                    <div className="w-full flex-1 min-h-[100px] mt-0.5">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={dynChartData} barCategoryGap="25%" barGap={4} margin={{ top: 6, right: 10, left: 0, bottom: 0 }}>
                                <XAxis dataKey="name" tick={{ fontSize: 9, fill: "#9ca3af", fontWeight: 600 }} axisLine={false} tickLine={false} />
                                <YAxis
                                ...
                                />
                                <Bar dataKey="costos" name="costos" fill="#E07A5F" radius={[3, 3, 0, 0]} maxBarSize={24} />
                                <Bar dataKey="ingresos" name="ingresos" fill="#2D6A4F" radius={[3, 3, 0, 0]} maxBarSize={24} />
                                {chartMode === "Por Campaña" && (
                                    <Bar dataKey="margen" name="margen" fill="#3B82F6" radius={[3, 3, 0, 0]} maxBarSize={24} />
                                )}
                            </BarChart>
                        </ResponsiveContainer>
                    </div>

                    <div className="flex flex-wrap gap-2.5 sm:gap-4 mt-1 shrink-0">
                        <div className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[#E07A5F] inline-block" /><span className="text-[9.5px] font-semibold text-gray-500 dark:text-gray-400">Inversión (Costos)</span></div>
                        <div className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[#2D6A4F] inline-block" /><span className="text-[9.5px] font-semibold text-gray-500 dark:text-gray-400">Ingresos (Cosecha)</span></div>
                        {chartMode === "Por Campaña" && (
                            <div className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[#3B82F6] inline-block" /><span className="text-[9.5px] font-semibold text-gray-500 dark:text-gray-400">Margen Neto Ganado</span></div>
                        )}
                    </div>
                </div>
            )}
...
function StatCard({ label, value, sub, subColor = "text-gray-400", icon, iconBg }) {
    return (
        <div className="bg-white dark:bg-[#1a1f25] rounded-xl p-2 sm:p-2.5 shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-0.5 sm:mb-1">
                <p className="text-[8.5px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest leading-tight">{label}</p>
                <div className={`w-5 h-5 ${iconBg} rounded-md flex items-center justify-center flex-shrink-0`}>{icon}</div>
            </div>
            <p className="text-lg sm:text-xl font-black text-gray-900 dark:text-gray-100 leading-none tracking-tight">{value}</p>
            <div className={`text-[9px] sm:text-[9.5px] font-semibold mt-0.5 ${subColor}`}>{sub}</div>
        </div>
    );
}