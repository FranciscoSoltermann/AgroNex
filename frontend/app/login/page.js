...
export default function AuthPage() {
    ...
    const validateForm = () => {
        ...
        if (!isLogin) {
            if (tipoUsuario === "FISICA") {
                ...
                if (!nombre.trim() || !nameRegex.test(nombre.trim())) {
                    return "El nombre no puede estar vacío y solo debe contener letras.";
                }
                if (!apellido.trim() || !nameRegex.test(apellido.trim())) {
                    return "El apellido no puede estar vacío y solo debe contener letras.";
                }
                
                const dniRegex = /^\d+$/; 
                if (!dni.trim() || !dniRegex.test(dni.trim())) {
                ...
                const cuitRegex = /^\d+$/; 
                if (!cuit.trim() || !cuitRegex.test(cuit.trim())) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#060D0B] font-sans selection:bg-green-100 antialiased">
            <Navbar />
            <div className="flex-1 flex overflow-hidden relative group">
                <div className="absolute inset-0">
                    ...
                    <div className="hidden lg:block absolute left-0 top-1/2 -translate-y-1/2 px-12 z-10">
                ...
                <div className="w-full md:w-2/5 md:ml-auto flex flex-col justify-center items-center p-4 sm:p-6 relative perspective-1000 z-10 overflow-y-auto">
                    <div className="flex flex-col items-center mb-6 z-20 mt-8 sm:mt-0">
                ...
                {showOtpChallenge && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                        <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl border border-gray-100 animate-in fade-in zoom-in duration-300">
                            ...
                            {otpError && (
                                <p className={`text-[11px] font-bold text-center mb-3 ${otpError.includes('✅') ? 'text-green-600' : 'text-red-600'}`}>{otpError}</p>
                            )}
