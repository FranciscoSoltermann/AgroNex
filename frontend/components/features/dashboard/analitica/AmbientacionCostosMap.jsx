                    ...
                    {ambientaciones.map((amb) => (
                        ...
                        >
                            ...
                            <div className="flex justify-between text-sm mt-3 border-t pt-2 border-gray-100">
                                ...
                                <span className="text-gray-600 flex items-center" title="Costo por Hectárea"><DollarSign className="w-4 h-4 mr-0 text-red-500"/> {amb.costoTotalInsumos}/ha</span>
                            </div>
                        </div>
                    ))}
                </div>
                                ...
                                {mapFeatures.map((f, i) => (
                                    <GeoJSON 
                                        ...
                                        style={() => ({
                                            ...
                                            opacity: 1,
                                            fillOpacity: selectedAmbientacion?.idAmbientacion === f.properties?.id ? 0.7 : 0.4
                                        })}
                                        eventHandlers={{
                                            click: () => {
                                                if (f.properties?.id) {
                                                    const amb = ambientaciones.find(a => a.idAmbientacion === f.properties.id);
                                                    if (amb) setSelectedAmbientacion(amb);
                                                }
                                            }
                                        }}
                                    />
